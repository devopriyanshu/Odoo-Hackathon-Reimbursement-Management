...
export default function ApprovalsPage() {
  ...
  const historyQuery = useApprovalsHistory();

  const { data: steps, isLoading } = activeTab === 'PENDING' ? pendingQuery : historyQuery;

  const handleAction = async (expenseId: string, action: 'APPROVE' | 'REJECT') => {
    if (action === 'REJECT' && commentOpenFor !== expenseId) {
    ...
    try {
      if (action === 'APPROVE') {
        await api.post(`/approvals/${expenseId}/approve`, { comment: comment.trim() || undefined });
      } else {
        await api.post(`/approvals/${expenseId}/reject`, { comment: comment.trim() });
      }
  ...
  const renderStepCard = (step: ApprovalStepWithExpense) => {
    const { expense } = step;
      ...
      >
        <div className="p-5 sm:p-6 grid grid-cols-1 md:grid-cols-4 gap-6 items-start">
          ...
          <div className="md:col-span-1 border-t md:border-t-0 md:border-l border-border pt-4 md:pt-0 md:pl-6 flex flex-col justify-center">
            ...
            {expense.currency !== expense.company?.currency && expense.company && (
              <p className="text-xs text-muted-foreground mt-1">
                Eqv.{' '}
                {new Intl.NumberFormat('en-US', {
                  style: 'currency',
                  currency: expense.company.currency,
                }).format(expense.amountInBase)}
              </p>
            )}
          </div>
          ...
          <div className="md:col-span-1 flex flex-col sm:items-end justify-center w-full gap-3 mt-2 md:mt-0">
            ...
            {activeTab === 'HISTORY' && step.comment && (
              <p className="text-xs text-muted-foreground italic text-right max-w-[200px]">
                "{step.comment}"
              </p>
            )}
          </div>
              ...
              <div className="p-5">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => { setCommentOpenFor(null); setComment(''); }}
    ...
    <PageContainer>
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-1 text-sm">
          Review and manage expense approval requests waiting for your action.
        </p>
      </div>
      ...
      <div className="mb-6 border-b border-border">
        <div className="flex gap-6 relative">
          <button
            onClick={() => { setActiveTab('PENDING'); setCommentOpenFor(null); setComment(''); }}
          ...
          <button
            onClick={() => { setActiveTab('HISTORY'); setCommentOpenFor(null); setComment(''); }}
            className={`pb-3 text-sm font-medium transition-colors relative ${
      ...
      {isLoading ? (
        ...
        <div className="space-y-4 pb-12">
          <AnimatePresence>{steps.map(renderStepCard)}</AnimatePresence>
        </div>
      )}
    </PageContainer>
  );
